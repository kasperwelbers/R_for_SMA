...
Note that you **need** to put `"wordcloud"` between quotation marks!
...
> **Question 1.c**. You decide to give every bar the favourite color of the respective teammember. You first create (assign) a new vector named `colornames` that contains these colors. (hint: this is just like how we made the `teammembers` vector above). Report your code.

> **Question 1.d**. Now give the `colornames` vector to the `col` argument in the barplot function. Provide a screenshot of the new colourful Bar chart as your answer. 
...
To start working with quanteda, we first need to run `library(quanteda)` to tell R that we want to use this package
...
```{r}
m = dfm(corp, remove=stopwords('english'), remove_punct=TRUE)
```
