...
We keep mentioning this R thing, but chances are you've never heard of it. R is an open-source statistical software language, that is currently [among the most popular](https://stackoverflow.blog/2017/10/10/impressive-growth-r/) software packages for data science. It's a popular for several reasons:
...
If this is the first time you're seeing R code , 
you might already get the basic gist of what's happening here. 
...
* *R* is the actual R software that is used to run R code. Technically, this is all you need, but it's not by itself nice to work with. You still need some software to write (or copy/paste) your code, and to view the results. Rather than using R by itself, it's therefore common to also install a `graphical user interface` (GUI). 
...
```{r, eval=F}
wordcloud("A very lame wordcloud")
```
...
You can recognize a function as a name followed by parentheses, e.g., `wordcloud("some text")`, `sum(1,2,3)`. 
The part between the parentheses contains the input to the function.
