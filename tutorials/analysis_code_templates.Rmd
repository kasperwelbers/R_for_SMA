...
```{r}
d = read_csv2('~/Downloads/obi4wan_export_20222806_100006.csv')
```
...
fix_stupid_date <- function(stupid_date, latest_year=2022) {
  ...
  year = ifelse(1:length(stupid_date) >= start_year, latest_year, latest_year-1)
...
```{r}
d$date = fix_stupid_date(d$Name, latest_year = 2022)
d
```
...
d$date = fix_stupid_date(d$Name, 2022)
d
```
...
```{r}
d = d %>% mutate(after_christmas = date >= '2021-12-26')
d
```
