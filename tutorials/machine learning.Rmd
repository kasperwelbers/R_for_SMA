...
```{r}
library(readr)
sentiment = read_csv('~/projects/r_for_sma/data/sentiment_tweets.csv')
sentiment
```
...
* use `docnames(dtm)` toget the document names. (we didn't provide names, so these are just 'text1','text2', etc.)
* draw a random sample of 5000 names.
* create `test_dtm` by subsetting the DTM to get the documents for which the name is in our test samlpe
* create `train_dtm` by subsetting the DTM to get document for which the name is NOT (!) in our test sample.
...
m = table(gold_standard = coded_validation_sample$manual_sentiment,
          predicted = coded_validation_sample$sentiment)
m
```
