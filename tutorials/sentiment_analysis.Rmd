...
So, a big limitation of rule-based approaches for sentiment analysis seem to be that we just can't seem to come up with a sufficiently complex set of rules.
...
dtm = beauty %>% corpus(text_field='text') %>% tokens() %>% dfm() %>%
  dfm_trim(min_termfreq=0) %>% dfm_remove(stopwords('en')) %>% dfm_wordstem() 
